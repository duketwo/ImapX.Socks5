...
namespace ImapX
{
    public class ImapBase : IDisposable
    {
        ...
        public bool Connect(string host, int port, SslProtocols sslProtocol = SslProtocols.None,
            bool validateServerCertificate = true, string socks5Host = "", int socks5Port = 1080, string socks5Username = null, string socks5Password = null)
        {
            _host = host;
            _port = port;
            ...
            try
            {
                ...
                {
                    var s5c = new Socks5Client(socks5Host, socks5Port, host, port, socks5Username, socks5Password);
                    if(!s5c.Connect())
                    {
                    ...
                    _client.Client = s5c.Client.Sock;
